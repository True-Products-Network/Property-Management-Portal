...
-- Date: July 31, 2026
-- VERSION 4 - All UUID types with proper RLS
...
-- NOTE: contact_roles references user_roles which must be UUID type
-- Run 20260731_fix_user_roles_types.sql first if needed
...
-- Get user's association IDs
CREATE OR REPLACE FUNCTION get_user_association_ids(check_user_id UUID)
RETURNS TABLE(assoc_id UUID) AS $$
BEGIN
    RETURN QUERY
...
CREATE POLICY "Management can manage properties" ON properties
    FOR ALL USING (
        EXISTS (
            ...
            WHERE user_id = auth.uid() 
            AND role IN ('ADMIN_USER', 'MANAGEMENT_STAFF')
            AND association_id = properties.association_id
            AND revoked_at IS NULL
        )
    );
...
CREATE POLICY "Users can view contacts in their associations" ON contacts
    FOR SELECT USING (
        EXISTS (
            ...
            AND cr.association_id IN (SELECT get_user_association_ids(auth.uid()))
        )
        OR is_admin_user(auth.uid())
    );
