-- Fix RLS policies for feature_flags table to work with Platform Admins
...
CREATE POLICY "feature_flag_overrides_select_policy" ON feature_flag_overrides
    FOR SELECT TO authenticated
    USING (true);

CREATE POLICY "feature_flag_overrides_insert_policy" ON feature_flag_overrides
    FOR INSERT TO authenticated
    WITH CHECK (is_platform_admin());

CREATE POLICY "feature_flag_overrides_update_policy" ON feature_flag_overrides
    FOR UPDATE TO authenticated
    USING (is_platform_admin());

CREATE POLICY "feature_flag_overrides_delete_policy" ON feature_flag_overrides
    FOR DELETE TO authenticated
    USING (is_platform_admin());
