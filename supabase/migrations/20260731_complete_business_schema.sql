...
-- Contact Roles (linking contacts to associations/properties/units with roles)
CREATE TABLE contact_roles (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    contact_id UUID NOT NULL REFERENCES contacts(id) ON DELETE CASCADE,
    
    role_type TEXT NOT NULL CHECK (role_type IN (
        'owner', 'co_owner', 'tenant', 'occupant', 
        'board_president', 'board_treasurer', 'board_secretary', 'board_member',
        'property_manager', 'assistant_manager', 'maintenance_staff',
        'vendor_contact', 'emergency_contact', 'other'
    )),
...
-- Unit Occupants (specific linking table for unit occupancy)
...
CREATE TABLE vendors (
    ...
    category TEXT CHECK (category IN (
        ...
        'Security', 'Pest Control', 'Roofing', 'Painting', 'General Contracting',
        'Elevator', 'Fire Safety', 'Pool Service', 'Snow Removal', 'Other'
    )),
...
CREATE TABLE maintenance_requests (
    ...
    -- Relationships (NO direct association_id - reach through property)
    ...
    category TEXT CHECK (category IN (
        ...
        'Cosmetic', 'Safety', 'Cleaning', 'Landscaping', 'Other'
    )),
    urgency TEXT CHECK (urgency IN ('emergency', 'urgent', 'normal', 'low')),
    
    -- Status workflow
    status TEXT DEFAULT 'new' CHECK (status IN (
        'new', 'triaged', 'pending_approval', 'approved', 'vendor_assigned',
        'scheduled', 'in_progress', 'on_hold', 'completed', 'closed', 'cancelled'
    )),
...
-- Maintenance Request Activity/Comments
CREATE TABLE maintenance_activity (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    maintenance_request_id UUID NOT NULL REFERENCES maintenance_requests(id) ON DELETE CASCADE,
    
    activity_type TEXT NOT NULL CHECK (activity_type IN (
        'status_change', 'comment', 'vendor_assigned', 'schedule_set',
        'cost_updated', 'completion', 'reopen', 'file_added'
    )),
...
CREATE TABLE inspections (
    ...
    inspection_type TEXT NOT NULL CHECK (inspection_type IN (
        'annual', 'move_in', 'move_out', 'fire_safety', 'elevator',
        'hvac', 'roof', 'pool', 'emergency_systems', 'insurance', 'other'
    )),
    
    status TEXT DEFAULT 'scheduled' CHECK (status IN (
        'scheduled', 'in_progress', 'completed', 'overdue', 'cancelled', 'rescheduled'
    )),
    ...
    follow_up_compliance_id UUID REFERENCES compliance_matters(id),
...
CREATE TABLE documents (
    ...
    file_path TEXT NOT NULL, -- Storage path (S3, etc.)
    file_size INTEGER,
    content_type TEXT,
    
    document_type TEXT CHECK (document_type IN (
        'insurance', 'financial', 'legal', 'meeting_minutes', 'contract',
        'inspection_report', 'certificate', 'policy', 'notice', 'other'
    )),
    ...
    -- Relationships (document can relate to multiple record types)
    ...
    compliance_matter_id UUID REFERENCES compliance_matters(id),
...
CREATE TABLE compliance_matters (
    ...
    category TEXT CHECK (category IN (
        'fire_safety', 'elevator', 'accessibility', 'environmental',
        'zoning', 'licensing', 'insurance', 'financial', 'other'
    )),
    
    priority TEXT CHECK (priority IN ('critical', 'high', 'medium', 'low')),
    status TEXT DEFAULT 'open' CHECK (status IN (
        'open', 'notice_issued', 'evidence_gathering', 'hearing_scheduled',
        'under_review', 'decision_pending', 'resolved', 'closed', 'appealed'
    )),
...
CREATE TABLE approvals (
    ...
    approval_type TEXT CHECK (approval_type IN (
        'maintenance', 'capital_improvement', 'vendor_contract',
        'budget_item', 'policy_change', 'assessment', 'other'
    )),
...
-- Approval Votes (for board voting)
...
CREATE TABLE payment_records (
    ...
    payment_type TEXT CHECK (payment_type IN (
        'assessment', 'special_assessment', 'late_fee', 'fine',
        'vendor_payment', 'deposit', 'other'
    )),
    
    amount DECIMAL(10,2) NOT NULL,
    
    -- Processor (NO card/bank details stored)
    ...
    payment_method_last4 TEXT, -- Last 4 digits only
...
-- Association Financial Accounts (masked info only)
CREATE TABLE association_accounts (
    ...
    -- Masked account info (last 4 only)
...
CREATE TABLE appointments (
    ...
    appointment_type TEXT CHECK (appointment_type IN (
        'maintenance', 'inspection', 'meeting', 'showing', 'vendor_visit', 'other'
    )),
...
-- Helper function to get user's association IDs
...
-- Associations: Users can view associations they belong to
...
-- Properties: Scoped through association
