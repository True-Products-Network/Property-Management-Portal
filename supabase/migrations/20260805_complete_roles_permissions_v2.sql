-- Complete roles/permissions setup v2
-- Run this if you get "column tenant_id does not exist" errors

-- First, verify roles table has tenant_id
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'roles' AND column_name = 'tenant_id' AND table_schema = 'public'
    ) THEN
        RAISE EXCEPTION 'roles table is missing tenant_id column';
    END IF;
END $$;
...
INSERT INTO roles (name, description, is_system_role, is_active, tenant_id)
SELECT 'Admin User', 'Full portal administrative access', true, true, NULL
WHERE NOT EXISTS (SELECT 1 FROM roles WHERE name = 'Admin User' AND tenant_id IS NULL);

INSERT INTO roles (name, description, is_system_role, is_active, tenant_id)
SELECT 'Portfolio Manager', 'Assigned Portfolio Operations and Management', true, true, NULL
WHERE NOT EXISTS (SELECT 1 FROM roles WHERE name = 'Portfolio Manager' AND tenant_id IS NULL);

INSERT INTO roles (name, description, is_system_role, is_active, tenant_id)
SELECT 'Association Manager', 'Assigned Association Management', true, true, NULL
WHERE NOT EXISTS (SELECT 1 FROM roles WHERE name = 'Association Manager' AND tenant_id IS NULL);

INSERT INTO roles (name, description, is_system_role, is_active, tenant_id)
SELECT 'Property Manager', 'Assigned Property Management', true, true, NULL
WHERE NOT EXISTS (SELECT 1 FROM roles WHERE name = 'Property Manager' AND tenant_id IS NULL);

INSERT INTO roles (name, description, is_system_role, is_active, tenant_id)
SELECT 'Board Member', 'Assigned Board view and approvals', true, true, NULL
WHERE NOT EXISTS (SELECT 1 FROM roles WHERE name = 'Board Member' AND tenant_id IS NULL);

INSERT INTO roles (name, description, is_system_role, is_active, tenant_id)
SELECT 'Vendor Contractor', 'Assigned Vendor Jobs', true, true, NULL
WHERE NOT EXISTS (SELECT 1 FROM roles WHERE name = 'Vendor Contractor' AND tenant_id IS NULL);

INSERT INTO roles (name, description, is_system_role, is_active, tenant_id)
SELECT 'Resident', 'Own associated records', true, true, NULL
WHERE NOT EXISTS (SELECT 1 FROM roles WHERE name = 'Resident' AND tenant_id IS NULL);

INSERT INTO roles (name, description, is_system_role, is_active, tenant_id)
SELECT 'Owner', 'Own associated records', true, true, NULL
WHERE NOT EXISTS (SELECT 1 FROM roles WHERE name = 'Owner' AND tenant_id IS NULL);

INSERT INTO roles (name, description, is_system_role, is_active, tenant_id)
SELECT 'Staff', 'Standard User Access', true, true, NULL
WHERE NOT EXISTS (SELECT 1 FROM roles WHERE name = 'Staff' AND tenant_id IS NULL);

INSERT INTO roles (name, description, is_system_role, is_active, tenant_id)
SELECT 'Finance User', 'Financial Access Only', true, true, NULL
WHERE NOT EXISTS (SELECT 1 FROM roles WHERE name = 'Finance User' AND tenant_id IS NULL);
