-- Fix RLS Recursion Issues - Version 4 (Correct table names)
-- Date: August 1, 2026
-- Uses CASCADE to force drop functions with all dependents
...
DROP FUNCTION IF EXISTS is_management_staff(UUID) CASCADE;

-- Also explicitly drop all policies to be safe (using correct table names)
...
DROP POLICY IF EXISTS "Users can view approvals in their associations" ON approvals;
DROP POLICY IF EXISTS "Management can manage approvals" ON approvals;
...
-- APPROVALS (corrected table name)
CREATE POLICY "Users can view approvals in their associations" ON approvals
    FOR SELECT USING (
        association_id::TEXT IN (SELECT get_user_association_ids(auth.uid()))
        OR is_admin_user(auth.uid())
    );

CREATE POLICY "Management can manage approvals" ON approvals
    FOR ALL USING (
        is_management_staff(auth.uid())
    );
...
-- PAYMENT RECORDS (corrected table name)
...
SELECT 'RLS policies fixed successfully with CASCADE' as result;
